...
void vaplus_get_index_stats(struct vaplus_index * index)
{
  ...
  // This implementation can operate as a flat serialized index without
  // populating the node-count macros. Fall back to a single-leaf view so
  // reporting remains meaningful for reproducibility runs.
  if (index->stats->total_ts_count == 0 && index->settings->dataset_size > 0)
  {
    index->stats->total_ts_count = index->settings->dataset_size;
  }
  if (index->stats->leaf_nodes_count == 0 && index->stats->total_ts_count > 0)
  {
    index->stats->leaf_nodes_count = 1;
  }
  if (index->stats->total_nodes_count == 0 && index->stats->leaf_nodes_count > 0)
  {
    index->stats->total_nodes_count = index->stats->leaf_nodes_count;
  }

  if (index->stats->leaf_nodes_count > 0 && index->stats->sum_fill_factor == 0)
  {
    double node_fill_factor = (index->stats->total_ts_count * 100.0) /
  ...
  if (index->stats->leaf_nodes_count > 0 && index->stats->sum_height == 0)
  {
    index->stats->min_height = 1;
    index->stats->max_height = 1;
    index->stats->sum_height = 1;
    index->stats->sum_squares_height = 1;
  }
...
void vaplus_get_index_footprint(struct vaplus_index * index)
{
    ...
    if (count_leaves > 0)
    {
      ...
      if (index->stats->sum_squares_fill_factor < 0)
      {
        index->stats->sum_squares_fill_factor = 0;
      }
      ...
      if (index->stats->sum_squares_height < 0)
      {
        index->stats->sum_squares_height = 0;
      }
...
void get_queries_stats(struct vaplus_index * index, unsigned int queries_count)
{
...
void print_queries_stats(struct vaplus_index * index, int count_queries)
{
  /*
        printf("---------------------------------------- \n");    
